...
def download_if_lastdownloaded_lt_lastModifiedDate(usr, password, db, url):
    ...
    curs = conn.cursor()
    curs.execute(sql,(url,),multi=True)
    try:
        ...
        previouslastModifiedDate = '2024-10-01T00:00:00-04:00'
    try:
...
def setup_database(db,usr,password):
    ...
    #-- nvd is a database in progress to pull CVE data from the NIST JSON tables
    nvd_schema = """
    ...
    curs = conn.cursor()
    if verbose:
      print(f"[.] Adding first schema..")
    curs.execute(db_schema,multi=True)
    conn.commit()
    conn.close()
    conn = mysql.connector.connect(
        host = mysqlip,
    ...
    curs = conn.cursor()
    if verbose:
      print(f"[.] Adding other schema for nvd, json, update_history, cpe, software_version, software_product, software_vendor ..")
    curs.execute(nvd_schema,multi=True)
    curs.execute(nvd_json_schema,multi=True)
    curs.execute(update_history_schema,multi=True)
    curs.execute(nvd_cpe,multi=True)
    curs.execute(cpe,multi=True)
    curs.execute(software_version,multi=True)
    curs.execute(software_product,multi=True)
    curs.execute(software_vendor,multi=True)
    conn.commit()
    conn.close()
    conn = mysql.connector.connect(
        host = mysqlip,
    ...
    try:
        if verbose:
          print(f"[.] Adding other schema, creating indexes on dates, etc..")
...
def insert_data_into_db(db,usr,password,data,source_url):
    ...
    try:
        curs.execute(sql,(source_url,download_date,lastModifiedDate,size,zipSize, gzSize,sha256),multi=True)
    except Exception as oops:
        print(oops)
    ...
    for cve in data['CVE_Items']:
        ...
        try:
            curs.execute('select id from nvd where cve_id = %s limit 1;',(cve_id,),multi=True)
            cur_id = curs.fetchone()
            if cur_id == None:
                curs.execute(insert_sql,(cve_id, description,configuration,vulnerable_software_list,impact,vector,access_complexity,authorize,confidentiality_impact,integrity_impact,availability_impact,published_date,modified_date,references),multi=True)
                conn.commit()
            # print("current id: %s cve_id: %s" % (cur_id,cve_id))
            else:
                curs.execute(replace_sql,(cur_id[0], cve_id, description,configuration,vulnerable_software_list,impact,vector,access_complexity,authorize,confidentiality_impact,integrity_impact,availability_impact,published_date,modified_date,references),multi=True)
                conn.commit()
        except Exception as oops:
        ...
        """
        try:
            curs.execute('select id from nvd_json where cve_id = %s limit 1;',(cve_id,),multi=True)
            cur_id = curs.fetchone()
            if cur_id == None:
                curs.execute(r'insert into nvd_json(cve_id, cve_item) values(%s,%s);', (cve_id,cve_json),multi=True)
                conn.commit()
            else:
                curs.execute(r'replace into nvd_json(id, cve_id, cve_item) values(%s, %s,%s);', (cur_id[0], cve_id,cve_json),multi=True)
                conn.commit()
        except Exception as oops:
...
def get_and_load(modifiers,filenametemplate,thisyear,db,user,password,baseurl):
    ...
    for modifier in modifiers:
        ...
        if download_if_lastdownloaded_lt_lastModifiedDate(user, password, db, url):
            #print(f"[!] skipping actual download of zip and extract.. {url} - {filename}")
            ...
            if os.path.isfile(filename):
              ...
              print(f"[!] file not found: {filename}, not loaded.")
